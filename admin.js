...
export function addProgramm() {
    ...
    let spielLink = document.getElementById("programm-url").value;

    if (download.includes("onedrive.live.com")) {
        download = download.replace("onedrive.live.com", "api.onedrive.com/rendition/thumbnail?url=onedrive.live.com").concat("&action=download");
    }

    if (titel && beschreibung && download) {
        push(ref(db, "programme"), { titel, beschreibung, download, spielLink })
            .then(() => {
...
export function addSpiel() {
    ...
    let spielLink = document.getElementById("spiel-url").value;
    ...
    if (titel && beschreibung && download) {
        push(ref(db, "spiele"), { titel, beschreibung, download, spielLink })
            .then(() => {
